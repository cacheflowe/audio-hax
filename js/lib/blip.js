...
BlipNode.prototype.param = function(name, f) {
  ...
  if (typeof f !== 'function') {
    if(typeof f !== 'undefined') this.node()[name].value = f;
  } else {
...
blip.clip = function() {
  ...
  var output_analyser = blip.node('analyser').connect(blip.destination);
  var output_gain = blip.node('gain').connect(output_analyser);

  // audio analysis stuff
  output_analyser.node().fftSize = 32;
// console.log(analyser.frequencyBinCount); // fftSize/2 = 32 data points

  var frequencyData = new Uint8Array(output_analyser.node().frequencyBinCount);

  function clip() {}

  clip.eqData = function() {
    output_analyser.node().getByteFrequencyData(frequencyData);
